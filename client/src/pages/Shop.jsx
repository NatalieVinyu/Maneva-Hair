    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-[#1a1218] px-6 py-16 text-center">
        <motion.div variants={stagger} initial="hidden" animate="visible">
          <motion.p className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-pink-300" variants={fadeUp} custom={0}>— The Collection —</motion.p>
      ...
      <div className="sticky top-0 z-20 border-b border-gray-100 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-4 px-6 py-4 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="border-0 bg-transparent text-xs font-medium text-gray-700 outline-none cursor-pointer">
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        <AnimatePresence mode="wait">
          <motion.div key={activeCategory + sortBy} className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
            variants={stagger} initial="hidden" animate="visible" exit={{ opacity: 0 }}>
            {filtered.map((wig, i) => (
              <motion.div key={wig.id} className="group relative cursor-pointer" variants={fadeUp} custom={i} layout>
                <div className="relative overflow-hidden rounded-sm bg-gray-100 aspect-[3/4]">
                  <motion.img src={wig.img} alt={wig.name} className="h-full w-full object-cover"
                    whileHover={{ scale: 1.05 }} transition={{ duration: 0.5 }} />
                  {wig.badge && (
                    <span className={`absolute left-3 top-3 rounded-full px-3 py-1 text-xs font-medium ${wig.badge === 'New' ? 'bg-pink-600 text-white' : 'bg-[#1a1218] text-white'}`}>
                      {wig.badge}
                    </span>
                  )}
                  <motion.button onClick={() => toggleWishlist(wig.id)}
                    className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-white/80 backdrop-blur-sm"
                    whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} aria-label="Save to wishlist">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4"
                      fill={wishlist.includes(wig.id) ? '#db2777' : 'none'}
                      viewBox="0 0 24 24" stroke={wishlist.includes(wig.id) ? '#db2777' : '#9ca3af'} strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                    </svg>
                  </motion.button>
                  <motion.div className="absolute inset-x-0 bottom-0 bg-[#1a1218]/90 p-4"
                    initial={{ y: '100%' }} whileHover={{ y: 0 }} transition={{ duration: 0.3, ease: 'easeOut' }}>
                    <Link to="/booking" className="block w-full rounded-full bg-pink-600 py-2 text-center text-xs font-medium text-white hover:bg-pink-700">
                      Book Install
                    </Link>
                  </motion.div>
                </div>
      ...
      <motion.section className="border-y border-gray-100 bg-gray-50 py-12"
        ...
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-px bg-gray-200 md:grid-cols-4">
          {[
            ...
            return (
              <motion.div key={item.label} className="flex flex-col items-center bg-gray-50 py-8 text-center" variants={fadeUp} custom={i}>
      ...
      <section className="relative overflow-hidden bg-pink-600 py-20 text-center">
        <motion.p className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 select-none font-serif text-[160px] font-light text-white/5 whitespace-nowrap"
          aria-hidden="true" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1.2 }} viewport={{ once: true }}>MANEVA</motion.p>
        <motion.div className="relative mx-auto max-w-xl px-6" variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <motion.h2 className="font-serif text-4xl font-light text-white" variants={fadeUp} custom={0}>Want a professional install?</motion.h2>
          <motion.p className="mt-3 text-sm text-white/60" variants={fadeUp} custom={1}>Our certified stylists will have you looking flawless. Book your slot today.</motion.p>
          <motion.div variants={fadeUp} custom={2}>
            <Link to="/booking" className="mt-7 inline-block rounded-full bg-white px-8 py-3 text-sm font-medium text-pink-600 hover:opacity-90">Book Appointment</Link>
          </motion.div>
        </motion.div>
      </section>
